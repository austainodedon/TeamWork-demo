...
export default class UserController {
  ...
  static async loginUser(req, res) {
    ...
    try {
      ...
      if (user) {
        const { firstName, id, lastName, password: userPassword } = user;
  ...
  /**
   * @method getUsers
   * @description - method  to get current users
   * @param {object} req - request object
   * @param {object} res - response object
   * @return {object} request response body
   */
  static async getUser(req, res) {
    try {
      const { id } = req.params;
      const { error, result: user } = await getItems("user", { id });
      if (error) {
        return errorResponse(res, 500, "Internal server error");
      }
      const noPasswords = user.map(user => {
        delete user.password;
        return user;
      });
      return successResponsArray(res, 200, noPasswords);
